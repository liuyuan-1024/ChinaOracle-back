-- 创建库
create
database if not exists chinaoracle;
...
(
    11
) not null comment '权限等级, 数值越大权限越大',
    `is_ban` tinyint
(
    1
) default 0 not null comment '0-未封禁 1-被封禁',
    `created_at` datetime default CURRENT_TIMESTAMP not null comment '创建时间',
...
(
    1
) default 0 not null comment '是否已删除(0-未删除 1-已删除)',
    primary key
(
    `id`
),
...
(
    11
) not null auto_increment comment '编程语言标签ID',
    `name` varchar
(
    64
) default '编程语言标签名称' not null comment '编程语言标签名称',
...
(
    11
) not null auto_increment comment '许可证ID',
    `name` varchar
(
    128
) default '许可证名称' not null comment '许可证名称',
    `url` varchar
(
    256
) default '许可证文本的URL地址' not null comment '许可证文本的URL地址',
...
(
    256
) default '仓库名称' not null comment '仓库名称',
...
(
    11
) default 1 not null comment '仓库主要使用的编程语言',
    `stars` int
(
    11
) default 0 not null comment '仓库的星标数',
    `forks` int
(
    11
) default 0 not null comment '仓库的分支数',
    `readme` text not null comment '仓库的README文件内容',
    `license_id` int
(
    11
) default 1 not null comment '开源许可证类型',
    `is_private` tinyint
(
    1
) default 0 not null comment '是否为私有仓库(0-公开 1-私有)',
    `is_deleted` tinyint
(
    1
) default 0 not null comment '是否已删除(0-未删除 1-已删除)',
